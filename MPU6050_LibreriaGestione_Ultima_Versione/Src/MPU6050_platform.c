...
HAL_StatusTypeDef MPU6050_VerificaComunicazioni(MPU6050_disp disp){
	...
	while(status != HAL_I2C_STATE_READY){
		...
		if((HAL_GetTick()-inizioTimer) >= STANDARD_TIMEOUT_MILLISEC){
			...
			status_ComUART = HAL_UART_Transmit(&(disp->perifericaComunicazioneHost), (uint8_t*)msg_errPer_MPU6050_VerificaComunicazioni, strlen(msg_errPer_MPU6050_VerificaComunicazioni), HAL_MAX_DELAY);
			break;
...
HAL_StatusTypeDef MPU6050_ImpostaDivisoreFrequenzaCampionamento(MPU6050_disp disp, uint8_t divisore){
		...
		if(divisore <= VALORE_MASSIMO_FREQUENZA_CAMPIONAMENTO){
...
HAL_StatusTypeDef MPU6050_LetturaDatoGrezzoAccelerometro__TrasformazioneDati(MPU6050_disp disp){
	...
	//effettuiamo una lettura sequenziale
	...
	if(status != HAL_OK){
		...
		//quando scrivi la relazione e parlerai di questa parte specifica che cosa stai facendo, e sopratutto che è corretto usare un buffer uint8_t e poi trasformarlo in int16_t
		...
		//fare un debugg qui e verficicare se il valore cicca fuori dal sensore
...
//LA DIREZIONE DEGLI ASSI A QUANTO PARE SEMBRA ESSERE INVERTITA, RISOLVIAMO IL PROBLEMA MOLTIPLICANDO IL DATO PER -1
			case FULL_SCALE_RANGE_2g :
					...
					//trasformazione del dato in metri al secondo
...
/*FUNZIONE DI DIPENDENZA CONTESTUALE NON PUò LAVORARE SE NON PER LA FUNZIONE SUCCESSIVA */

//i valori di DLPF e SMPLR li prendiamo direttamente all'interno della funzione magari creando altre due funzioni
//che non sono di dipendenza contestuale
...
/*STEP DI MIGLIORAMENTE : CALCOLARE IL VALORE ESATTO DI ESCURSIONE A SENSORE FERMO IN MODO DA EVITARE MISURE CHE IN REALTA SONO BUONO
	 * PER OGNI ASSE E PER OGNI FULL_SCALE_RANGE
	 * CALCOLARE MATEMATICAMENTE IL VALORE DI AMPLIFICAZIONE DEL SEGNALE SU TANTE PROVE
	 * SISTEMARE L'ERRORE
	 * */
//questa diventerà la nostra funzione dipendente dal contesto, contestuale
	...
	if(status != HAL_OK){
		...
		case FULL_SCALE_RANGE_2000_gr_sec :
				...
				if((disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_X) > VALORE_MAX_VEL_ANG_MISURABILE_CON_RUMORE_PER_2000_g_s){
					...
					disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_X = (disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_X-ESCURSIONE_GIRO_2000_g_s);

				}

				if((disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_Y) > VALORE_MAX_VEL_ANG_MISURABILE_CON_RUMORE_PER_2000_g_s){

					disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_Y = (disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_Y-ESCURSIONE_GIRO_2000_g_s);
				}

				if((disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_Z) > VALORE_MAX_VEL_ANG_MISURABILE_CON_RUMORE_PER_2000_g_s){

					disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_Z = (disp->datiGiroscopio.datoVelAngGiro.velocitaaAngolare_Z-ESCURSIONE_GIRO_2000_g_s);
				}
