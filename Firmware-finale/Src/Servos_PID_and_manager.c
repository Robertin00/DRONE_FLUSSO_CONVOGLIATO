...
/*------------------------------------- CLOCKWISE SECTION -------------------------------------*/
		...
		if(angle_value <= 0){
			...
			new_int_term_cl_wi = int_term + ((pid_pars_pointer->int_coeff*pid_pars_pointer->sampl_time)/2)*(e_old+e);

			float der_term_cl_wi = (pid_pars_pointer->der_coeff/pid_pars_pointer->sampl_time)*(e-e_old);

			e_old = e;

			u = CENTER_UPPER_LOWER_SERVO_SATURATION-(prop_term_cl_wi + new_int_term_cl_wi + der_term_cl_wi);
...
/*------------------------------------- COUNTERCLOCKWISE SECTION -------------------------------------*/
		...
		if(angle_value > 0){
			...
			new_int_term_co_cl_wi = int_term + ((pid_pars_pointer->int_coeff*pid_pars_pointer->sampl_time)/2)*(e_old+e_2);

			float der_term_co_cl_wi = (pid_pars_pointer->der_coeff/pid_pars_pointer->sampl_time)*(e_2-e_old);

			e_old = e_2;

			u = CENTER_UPPER_LOWER_SERVO_SATURATION+(prop_term_co_cl_wi + new_int_term_co_cl_wi + der_term_co_cl_wi);
...
void servos_turn_off(void){
	...
	TIM5->CCR4 = CENTER_UPPER_LOWER_SERVO_SATURATION;
