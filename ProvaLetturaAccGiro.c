...
void MPU6050_Init(void){
	...
	//la lettura è sequenziale è può essere fatta direttamente da un unico registro
void MPU6050_LettoreAccelerazione(void){
		HAL_I2C_Mem_Read(&hi2c2,MPU6050_ADDR, ACCEL_XOUT_H_REG, 1, recData1,6,1000);

		accelerazioneGrezzaX = (int16_t)(recData1[0]<<8 | recData1[1]);
		accelerazioneGrezzaY = (int16_t)(recData1[2]<<8 | recData1[3]);
		accelerazioneGrezzaZ = (int16_t)(recData1[4]<<8 | recData1[5]);

		AX = accelerazioneGrezzaX/16384.0;
		AY = accelerazioneGrezzaY/16384.0;
		AZ = accelerazioneGrezzaZ/16384.0;

	}

void MPU6050_LettoreVelAngolare(void){
	HAL_I2C_Mem_Read(&hi2c2, MPU6050_ADDR, GYRO_XOUT_H_REG,1, recData2,6,1000);

	gyroGrezzaX = (int16_t)(recData2[0]<<8 | recData2[1]);
	gyroGrezzaY = (int16_t)(recData2[2]<<8 | recData2[3]);
	gyroGrezzaZ = (int16_t)(recData2[4]<<8 | recData2[5]);

	Gx = gyroGrezzaX/131.0;
	Gy = gyroGrezzaY/131.0;
	Gz = gyroGrezzaZ/131.0;
